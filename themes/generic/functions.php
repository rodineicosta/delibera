    ...
    if (get_post_type($post) == "pauta") {
        ...
        switch ($situacao->slug)
        {
            ...
            case 'relatoria':
            {
                ...
                if (class_exists('\Delibera\Includes\SideComments\CTLT_WP_Side_Comments')  // have side comment module
                    ...
                    if (delibera_current_user_can_participate()) {
                        ...
                        case 'radio':
                            ...
                            foreach ($encaminhamentos as $encaminhamento)
                            {
                                ...
                                if (!empty($hasbasedon) && !$show_based_proposals) {
                                    continue;
                                }

                                $hasbasedon_class = empty($hasbasedon) ? '' : 'delibera-hasbasedon';

                                if (!array_key_exists($encaminhamento->comment_author, $users)) {
                                    $users[$encaminhamento->comment_author] = 0;
                                }
...
function delibera_generateProposalPair($proposals, $form = "", $users = array(), $post_id = false)
{
    ...
    {
        if (!in_array($encaminhamento->comment_ID, $pair)) {
            continue;
        }
