...
function delibera_mailer_reply($id)
{
    ...
    $comment_post   = 0;
    $isreply        = 0;
    if ((int) esc_sql($_POST['comment_parent']) === 0 || (int) esc_sql($_POST['comment_post_ID']) === 0) {
...
function delibera_mailer_mailer($id,$comment_parent,$comment_post, $tipo = 'reply')
{
    ...
    if ($post->post_status == 'publish') {
        ...
        if ($options_plugin_delibera["reposta-por-email"] == "N" // Notificações estão desabilitadas OU
      ) {
            return false;
        }
