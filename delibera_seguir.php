...
function delibera_seguir($ID, $type = 'seguir')
{
    ...
    if ($type == 'seguir') {
        ...
        if (!is_array($seguiram)) {
            $seguiram = array();
        }
        $hora = time();
        if (!array_key_exists($hora, $seguiram)) {
            $seguiram[$hora] = array();
        }
        ...
        foreach ($seguiram as $hora => $segs)
        {
            foreach ($segs as $user_ip)
            {
                if ($user_id != $user_ip['user']) {
                    if (!array_key_exists($hora, $seguiram2)) {
                        $seguiram2[$hora] = array();
                    }
    ...
    case 'string':
        ...
        foreach ($seguiram_hora as $hora => $seguiram)
        {
            foreach ($seguiram as $seguiu)
            {
                ...
                $ret .= (($seguiu['user'] == false || $seguiu['user'] == 0) ? $seguiu['ip'] : get_the_author_meta($seguiu['user']));
            }
        }
        return $ret;
        break;
